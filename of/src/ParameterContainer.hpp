...
// class ofVec2f;
// class ofVec3f;
// #define NEED_ENCLOSE_IN_BRACKET(T) (std::is_same<T,ofVec2f>::value  || std::is_same<T,ofVec3f>::value)
...
public:
  ...
  void stateFromString(const string & s) override{
    setValue(StringUtils::fromString<T>(s));
  ...
  void notifyValueChanged(){ if(!isCommiting){
    // do stuff
